...
def registerPlayer(name):
    ...
    db = connect()
    cur = db.cursor()
    cur.execute("insert into tournament01.player_records (name) VALUES (%s);",(name,))
    db.commit()
    cur.execute("select * from tournament01.player_records ")
    db.close()
...
def playerStandings():
    ...
    db = connect()
    cur = db.cursor()
    cur.execute("select id, name, wins, matches from players_standing ORDER BY wins DESC, matches ASC ")
    result = cur.fetchall()
    db.close()
    return result
...
def reportMatch(winner, loser):
    ...
    db = connect()
    c = db.cursor()
    c.execute("insert into tournament01.player_matches values (%s, %s)", (winner, loser))
    db.commit()
    db.close()
